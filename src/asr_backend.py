"""Бэкенды распознавания речи через OpenAI-совместимый API (Groq/OpenAI): транскрипция и judge-модель для метрик WER/CER."""
...
def _load_asr_model(
    *,
    ...
    if provider in {"groq", "openai"}:
        if OpenAI is None:
            raise ImportError(
                "Для ASR через OpenAI-compatible API требуется пакет openai."
            )

        api_key = os.getenv(api_key_env, "").strip()
        if not api_key:
            raise EnvironmentError(
                f"Для ASR через {provider} установите переменную окружения "
                f"{api_key_env}."
            )

        logger.info(
            "Используем %s ASR %s через %s для %s...",
            provider,
            api_model_name,
            base_url,
            purpose,
