"""Аудио-обработка TTS: согласование уровня сегментов с оригиналом, потолок пиков и финальная обработка дорожки дубляжа."""
...
from src.audio_analysis import active_speech_dbfs, active_speech_stats
...
def _match_segment_level(
    ...
    max_boost_db: float,
    max_cut_db: float,
    speech_stats: Dict[str, float | None] | None = None,
) -> AudioSegment:
    ...
    current_active_dbfs = (
        speech_stats.get("active_dbfs")
        if speech_stats is not None
        else _active_speech_dbfs(audio)
    )
