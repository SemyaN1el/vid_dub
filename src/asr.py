"""Распознавание речи (Whisper) с пословными таймингами, сегментация по паузам и построение multi-reference голосового профиля диктора с routing-пулом референсов."""
...
from src.audio_analysis import active_speech_stats
...
def _active_speech_stats(audio: AudioSegment) -> Dict[str, float | None]:
    ...
    return active_speech_stats(audio, fallback_active_ratio=0.0)
...
def transcribe_and_segment(
    ...
    """
    language = (asr_language or "").strip() or None
