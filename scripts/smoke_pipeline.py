...
def validate_artifacts(
    ...
    _require_file(
        output_dir / "translated_segments.clean.json",
        "translated_segments.clean.json",
    )
    _require_file(
        output_dir / "translation_quality.json",
    ...
    if (tts_provider or "").strip().lower() == "elevenlabs":
        _require_file(paths["elevenlabs_voice"], "elevenlabs_voice.json")
...
def build_main_command(args: argparse.Namespace, video_path: Path, job_name: str) -> list[str]:
    ...
    if args.mt_provider:
        cmd.extend(["--mt-provider", args.mt_provider])
    if args.mt_strategy:
        cmd.extend(["--mt-strategy", args.mt_strategy])
    if args.tts_provider:
        cmd.extend(["--tts-provider", args.tts_provider])
    if args.elevenlabs_voice_id:
        cmd.extend(["--elevenlabs-voice-id", args.elevenlabs_voice_id])
    if args.elevenlabs_voice_name:
        cmd.extend(["--elevenlabs-voice-name", args.elevenlabs_voice_name])
    if args.elevenlabs_no_clone:
        cmd.append("--elevenlabs-no-clone")
    if args.resume:
        cmd.append("--resume")
        for force_step in args.force_step:
            cmd.extend(["--force-step", force_step])
    return cmd
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help="Optional translation provider override passed to main.py.",
    )
    parser.add_argument(
        "--mt-strategy",
        choices=["per-segment", "sentence-boundary-aware"],
        default=None,
    ...
    parser.add_argument(
        ...
        default=None,
        help="Optional TTS provider override passed to main.py.",
    )
    parser.add_argument(
        "--elevenlabs-voice-id",
        default=None,
        help="Optional existing ElevenLabs voice_id passed to main.py.",
    )
    parser.add_argument(
        "--elevenlabs-voice-name",
        default=None,
        help="Optional ElevenLabs cloned voice name passed to main.py.",
    )
    parser.add_argument(
        "--elevenlabs-no-clone",
        action="store_true",
        help="Pass --elevenlabs-no-clone to main.py.",
    )
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Pass --resume to main.py.",
    )
    parser.add_argument(
        "--force-step",
        choices=["preprocess", "asr", "translate", "tts", "postprocess", "subtitles", "metrics", "prepare_finetune", "all"],
        action="append",
        default=[],
        help="Pass one or more --force-step values to main.py when --resume is used.",
    )
    parser.add_argument(
        "--skip-run",
        action="store_true",
