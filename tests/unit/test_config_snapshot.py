...
def test_build_tts_config_snapshot_groups_key_tts_settings() -> None:
    config = SimpleNamespace(
        ...
        ELEVENLABS_CLONE_VOICE=True,
        SMART_SYNC_ENABLED=True,
        SMART_SYNC_PROVIDER="groq",
        SMART_SYNC_MAX_REWRITES=1,
        SMART_SYNC_TARGET_SPEEDUP_FACTOR=1.25,
    ...
    assert snapshot["elevenlabs"]["model_id"] == "eleven_multilingual_v2"
    assert snapshot["elevenlabs"]["voice_id_configured"] == "voice_123"
    ...
    assert snapshot["smart_sync"]["target_speedup_factor"] == 1.25
    assert snapshot["runtime"]["grouping_enabled"] is True
    assert snapshot["segment_routing"]["enabled"] is False
...
def test_build_pipeline_config_snapshot_exposes_runtime_sections_without_secrets() -> None:
    ...
    assert snapshot["subtitles"]["mode"] == "hard"
